...
from datasets.seco_datamodule import SeasonalContrastBasicDataModule, \
...
def get_experiment_name(hparams):
    ...
    name = f'{hparams.base_encoder}-{data_name}-{hparams.data_mode}-epochs={hparams.max_epochs}-{hparams.description}'
    return name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_mode', type=str, choices=['moco', 'moco_tp', 'seco', 'teco', 'caco'], default='seco')
    ...
    elif args.data_mode == 'teco':
        datamodule = TemporalContrastMultiAugDataModule(
            data_dir=args.data_dir,
            batch_size=args.batch_size,
            num_workers=args.num_workers
        )
    elif args.data_mode == 'caco':
        datamodule = ChangeAwareContrastMultiAugDataModule(
            data_dir=args.data_dir,
            batch_size=args.batch_size,
            num_workers=args.num_workers
        )
    ...
    trainer = Trainer.from_argparse_args(
        ...
        callbacks=[scheduler, online_evaluator, HistogramCallback(what='epochs', datamodule=datamodule)],
