...
class SeasonalContrastBase(Dataset):
    ...
    def samples(self):
        if self._samples is None:
            ...
            self.loss = [0 for tmp in range(len(self._samples)*2)]
            self.lossdeno = [0 for tmp in range(len(self._samples)*2)]
            self.gmmlabel = [0 for tmp in range(len(self._samples)*2)]
...
class SeasonalContrastMultiAug(SeasonalContrastBase):
    ...
    def __getitem__(self, index):
        ...
        return index, q, [k0, k1, k2]


class TemporalContrastMultiAug(SeasonalContrastBase):
    ...
    def __getitem__(self, index):
        root = self.samples[index//2]
        first = index%2
        # first five images and last 5 images have a gap of years
        if first:
            sorted_paths = sorted([path for path in root.glob('*') if path.is_dir()], reverse=True)[:5]
        else:
            sorted_paths = sorted([path for path in root.glob('*') if path.is_dir()], reverse=True)[5:]
        choice = np.random.choice(sorted_paths, 3)

        t0, t1, t2 = [read_image(path, self.bands, QUANTILES) for path in choice]
...
class ChangeAwareContrastMultiAug(SeasonalContrastBase):
    augment = transforms.Compose([
        transforms.RandomApply([
            transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)  # not strengthened
        ], p=0.8),
    ...
    def samples(self):
        if self._samples is None:
            self._samples = self.get_samples()
            self.loss = np.array([[0.0, 0.0, 0.0] for tmp in range(len(self._samples)*2)])
            self.lossdeno = np.array([0 for tmp in range(len(self._samples)*2)])
            self.gmmlabel = np.array([0 for tmp in range(len(self._samples)*2)])
        return self._samples
    ...
    def __getitem__(self, index):
        ...
        choice = np.random.choice(sorted_paths, 3, replace=False)
        ...
        if self.gmmlabel is None:
            mmweight=0
        else:
            if self.gmmlabel[index]==1:
                mmweight=1
            else:
                mmweight=0
        return index, q, [k0, k1, k2], mmweight